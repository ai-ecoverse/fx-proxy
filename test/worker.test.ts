...
describe("router", () => {
  it("serves health with CORS", () => {
    const r = callWorker({ method: "GET", url: "https://x.test/health", env: { SEARCH_PROVIDER: "ddg" } });
  ...
  it("serves the model list", () => {
    const r = callWorker({ method: "GET", url: "https://x.test/v1/models", env: { DEFAULT_MODEL: "m/x" } });
    expect(r.status).toBe(200);
    expect((r.json as any).data[0].id).toBe("m/x");
  });

  it("answers preflight", () => {
    const r = callWorker({ method: "OPTIONS", url: "https://x.test/v1/responses" });
    expect(r.status).toBe(204);
    expect(r.headers["access-control-allow-methods"]).toContain("POST");
  });

  it("404s unknown routes", () => {
    const r = callWorker({ method: "GET", url: "https://x.test/nope" });
    expect(r.status).toBe(404);
    expect((r.json as any).error.message).toContain("unknown route: GET /nope");
  });

  it("405s wrong methods with an allow header", () => {
    const r = callWorker({ method: "GET", url: "https://x.test/v1/responses" });
    expect(r.status).toBe(405);
    expect(r.headers.allow).toBe("POST, OPTIONS");
  });

  it("rejects an unsupported search provider", () => {
    const r = callWorker({ method: "GET", url: "https://x.test/health", env: { SEARCH_PROVIDER: "bing" } });
...
describe("credentials", () => {
  ...
  it("uses its own gateway key when the proxy key matches", () => {
    let auth = "";
    const r = callWorker({
